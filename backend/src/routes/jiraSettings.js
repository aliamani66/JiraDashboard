  ...
  try {
    ...
    res.json({
      ...
      confluence: {
        ...
        defaultSpaceKey: env.CONFLUENCE_DEFAULT_SPACE || 'OPS',
      },
      waitingStatuses: (env.JIRA_WAITING_STATUSES || 'OnHolding,Waiting,Blocked,On Hold').split(',').map(s => s.trim()),
      statusMapping: jiraMapping.statusMapping || {},
      customFields: {
        sprintField: env.JIRA_SPRINT_FIELD || 'customfield_10020',
        waitingTeamField: env.JIRA_WAITING_TEAM_FIELD || '',
        waitingReasonField: env.JIRA_WAITING_REASON_FIELD || '',
        confluenceLinkField: env.JIRA_CONFLUENCE_LINK_FIELD || '',
        capabilitiesField: env.JIRA_CAPABILITIES_FIELD || '',
        categoryField: env.JIRA_CATEGORY_FIELD || '',
      },
      dateMapping: {
        epicStartDateField: env.JIRA_EPIC_START_DATE_FIELD || 'created',
        epicDueDateField: env.JIRA_EPIC_DUE_DATE_FIELD || 'duedate',
        taskStartDateField: env.JIRA_TASK_START_DATE_FIELD || '',
        taskDueDateField: env.JIRA_TASK_DUE_DATE_FIELD || 'duedate',
      },
      labelPrefixes: {
        waitingTeam: env.JIRA_WAIT_TEAM_PREFIX || 'wait:',
        waitingReason: env.JIRA_WAIT_REASON_PREFIX || 'reason:',
        capability: env.JIRA_CAPABILITY_PREFIX || 'cap:',
      },
      featuredComponents: (env.JIRA_FEATURED_COMPONENTS || 'learning,meeting,support').split(',').map(s => s.trim()),
    });
  } catch (err) {

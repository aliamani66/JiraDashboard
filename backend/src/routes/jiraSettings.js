...
router.post('/run-tests', async (req, res) => {
  try {
    const { runSystemTestSuite } = require('../services/systemTestRunner');
    const results = await runSystemTestSuite();
    return res.json(results);
  } catch (err) {
    console.error('Error running automated test suite:', err);
    res.status(500).json({ success: false, message: 'خطا در اجرای آزمون‌های سیستم: ' + err.message });
  }
});

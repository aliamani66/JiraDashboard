...
$remoteCmd = "export PATH=`$PATH:/usr/local/bin:/usr/bin; DC='docker compose'; if command -v docker-compose >/dev/null 2>&1; then DC='docker-compose'; fi; if docker compose version >/dev/null 2>&1; then DC='docker compose'; fi; mkdir -p /appserver/amani/JiraDashboard && tar -xzf /tmp/JiraDashboard.tar.gz -C /appserver/amani/JiraDashboard && cd /appserver/amani/JiraDashboard && (`$DC down || true) && `$DC build --no-cache && `$DC up -d --force-recreate"

...
$remoteCmd = "mkdir -p /appserver/amani/JiraDashboard && tar -xzf /tmp/JiraDashboard.tar.gz -C /appserver/amani/JiraDashboard && cd /appserver/amani/JiraDashboard && docker compose down && (docker compose build --no-cache && docker compose up -d --force-recreate || docker-compose build --no-cache && docker-compose up -d --force-recreate)"

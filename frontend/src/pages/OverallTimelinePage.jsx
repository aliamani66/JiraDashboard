...
import { TrendingUp, Layers, ArrowLeft, ExternalLink, Activity, Clock, CheckCircle2, AlertTriangle } from 'lucide-react';
...
const OverallTimelinePage = () => {
  ...
  const totalSpentHours = projects.reduce((sum, p) => sum + (p.total_spent_hours || 0), 0);
  const totalEstimateHours = projects.reduce((sum, p) => sum + (p.total_estimate_hours || 0), 0);
  ...
  // Generate synthetic trajectory for each project across 10 sprints based on current progress
  const colors = ['#10B981', '#3B82F6', '#F97316', '#8B5CF6', '#EC4899', '#06B6D4', '#EAB308'];

  const projectCurves = projects.map((proj, pIdx) => {
    ...
    const points = sprintLabels.map((_, sIdx) => {
      ...
      // Curved trajectory with slight variation
